...
{
    public class ShinigamiForm : STS_Komachi_OnozukaCard
    {

        public override bool CanBeGeneratedInCombat => false;
        public ShinigamiForm()
        ...
        protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
        {
            ...
            if (IsUpgraded)
            {
                CardModel token = CombatState.CreateCard<ManipulateDistanceToken>(Owner);
                await CardPileCmd.Add(token, PileType.Hand);
            }

            await PowerCmd.Apply<ShinigamiFormPower>(
